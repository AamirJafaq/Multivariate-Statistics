...
#we chose a dataset with information about percentage of coverage of vaccines  | 
...
cor_brazil_china <- cca_brazil_china$cor[1]                 #
cor_brazil_india <- cca_brazil_india$cor[1]                 #
cor_brazil_mexico <- cca_brazil_mexico$cor[1]               #
cor_china_india <- cca_china_india$cor[1]                   #
cor_mexico_china <- cca_mexico_china$cor[1]                 #
cor_mexico_india <- cca_mexico_india$cor[1]                 #
#                                                           #
#Printing the max correlations:                             #
print(paste("Brazil-Mexico:", cor_brazil_mexico))           #
print(paste("Brazil-China:", cor_brazil_china))             #
print(paste("Brazil-India:", cor_brazil_india))             #
print(paste("Mexico-China:", cor_mexico_china))             #
print(paste("Mexico-India:", cor_mexico_india))             #
print(paste("China-India:", cor_china_india))               #
#____________________________________________________________
...
# years, or due an influence by the nature of the can variates themselves.    |
...
# In the paper, we are following method 1, we shouldn´t consider the corr with|
# Brazil and Mex involved but we don't have issues like that in our data as...|
#______________________________________________________________________________
...
# Convert Ut to a data frame if it's not already
Ut <- as.data.frame(Ut)
...
# Reshape the data for plotting with ggplot2
U_j_long <- melt(data.frame(time, U_j), id.vars = 'time')
...
#______________________________________________________________________________
# Now, we can use these functions to compute get the cross-autocorrelation    |
# matrices formatrix ρ^(k) for a specific lag k by looping over all possible  |
...
# Print the tables
print("Cross-Autocorrelation Matrix for k=0:")
print(df_rho_0)

print("Cross-Autocorrelation Matrix for k=1:")
print(df_rho_1)

print("Cross-Autocorrelation Matrix for k=2:")
print(df_rho_2)
...
print(is_pd)                                                #
                                                            #
rho_1_abs <- make_positive_definite_abs(rho_1)              #
is_pd <- is_positive_definite(rho_1_abs)                    #
print(is_pd)                                                #
                                                            #
# FOR k=2                                                   #
...
rho_2_ab <- make_positive_definite(rho_2_ab)                #
rho_2_abs <- make_positive_definite_abs(rho_2_ab)           #
is_pd <- is_positive_definite(rho_2_abs)                    #
print(is_pd)                                                #
#____________________________________________________________
...
# Eigen decomposition of rho_1_abs
eigen_results <- eigen(rho_1_abs)
...
ggplot(pc_data, aes(x = PC1, y = PC2, color = Country)) +
  geom_point() +
  theme_minimal() +
  labs(title = "Scatter plot of the first two principal components with country colors",
...
colors <- c("black", "green4","purple", "blue4")
